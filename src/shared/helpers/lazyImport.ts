...
export default function lazyImport<T extends ComponentType<any>>(component: LazyImportComponent<T>, timeout = 800): LazyExoticComponent<T> {
    const LazyComponent = lazy<T>(async () => {
        let importedComponent: LazyImportReturnType<T>;

        if (__IS_DEV__) {
            importedComponent = await new Promise((resolve) => {
                setTimeout(async () => resolve(await component()), timeout);
            });
        } else {
